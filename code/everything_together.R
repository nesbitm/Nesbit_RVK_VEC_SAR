...
rma1 = data.frame(rlog =T_rma1,
  ...
  annotate(geom="text", x=0, y=-2, label = 'RMA values: y = 0.273x + 1.881, p = 0.044')
...
ggplot(CFU2, aes(x= rlog, y= Klog, color = Temp))+
  ...
  annotate(geom="text", x=0, y=-2, label = 'RMA values: y = 0.354x + 2.354, p = 5.29e-08')


lmodel3 <- lmodel2(Klog~Temp, data = CFU2, range.y = 'interval', range.x = 'interval')
rma<- list()
T_rma = seq(min(CFU2$Temp), max(CFU2$Temp), length.out = 100)
rma = data.frame(Temp =T_rma,
                 rmapred =(0.01991239*T_rma) +	1.1755627 )

CFU2 <- CFU2 %>%
  mutate(Temperature = Temp)

#plot
ggplot(CFU2, aes(x = Temp, y= Klog))+
  geom_point(size = 5)+ 
  ggtitle("CFU")+
  xlab('Temperature (C)')+
  ylab('log(K)')+
  ...
  #facet_wrap(~spp_species, scales = 'free')
  geom_line(data = rma, aes(y = (rmapred)), color = 'red', size = 3)+
  annotate(geom="text", x=30, y=-2, label = 'y = 0.0199x + 1.175, p = 0.042')
...
ggplot(fakedata2, aes(x= Fake_Data_X, y= Fake_Data_Y, color = Style))+
  ...
  ggtitle("Predicted Graphs from Different Hypotheses")+
  xlab('Temperature (C)')+
  ylab('r or K')+
  geom_line(aes(x= Fake_Data_X, y= Fake_Data_Y, color = r.K, size = 2))+
  guides(size = 'none')+
  geom_dl(aes(label = r.K),color = 'black', method = list(dl.combine("last.points"), cex = 1 ))+
...
fakedata3 <- fakedata3 %>%
  mutate(Hypothesis = Style)

#plot
ggplot(fakedata3, aes(x= Fake_Data_X, y= Fake_Data_Y, color = Hypothesis))+
  # geom_point(size = 2)+ 
  # facet_wrap(~Style)+
  ggtitle("Predicted Graphs from Different Hypotheses")+
  xlab('Temperature (C)')+
  ylab('log(r) - log(K) = log(a)')+
  geom_smooth(method='lm', formula= y~x, se =F, size =5)+ 
  #geom_line(aes(x= Fake_Data_X, y= Fake_Data_Y, color = Style, size = 5))+
  guides(size = 'none')+
theme( axis.text.x=element_blank(),
       axis.ticks.x=element_blank())+
  theme( axis.text.y=element_blank(),
...
CFU=CFU %>%
mutate(Temperature = (1 / (k * (Temp+273.15))) - (1 / (k * (15+273.15))))

CFU <- CFU %>%
  mutate(a = r / K)


 #  ggplot(data = CFU, aes(Temp2,log(a))) +
 #  geom_point(size= 3)+
 #  facet_wrap(~spp_species,scales="free")+
 #  # geom_hline(aes(yintercept = a0), data = bind_rows(res))
 # geom_line(data = bind_rows(res), aes( y = (log_a_pred)), color = 'red', size = 1.5)+
